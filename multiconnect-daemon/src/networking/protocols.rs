      ...
      Some(ConnectionState::Disconnected) | None => {
        if let Some(addr) = self.peer_addrs.get(&peer_id) {
          ...
          self.pending_events.push_back(ToSwarm::Dial {
            opts: DialOpts::peer_id(peer_id).addresses(vec![addr.clone()]).extend_addresses_through_behaviour().build(),
          });
  ...
  fn on_swarm_event(&mut self, event: libp2p::swarm::FromSwarm) {
    match event {
      ...
      libp2p::swarm::FromSwarm::ConnectionClosed(connection_closed) => {
        ...
        self.pending_events.push_back(ToSwarm::GenerateEvent(BehaviourEvent::StreamClosed {
          peer_id,
          reason: StreamCloseReason::RemoteClosed,
        }));
      }
      _ => {}
    }
